...
public class NumberGtRestrictionGenerator<T extends Number> extends ComplexRestrictionGeneratorBase<T> {
    public NumberGtRestrictionGenerator(String fieldName, T number) {
    ...
    protected List<Predicate> doGenerate(CriteriaBuilder criteriaBuilder, Root<?> entity, String field, Iterable<Expression<T>> expressions) {
        ...
        return Arrays.asList(criteriaBuilder.gt(entity.get(field), expression));
    }
}
